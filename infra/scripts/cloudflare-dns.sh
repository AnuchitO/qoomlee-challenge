#!/usr/bin/env bash
# Add/update Cloudflare DNS A records for a team's dev environment.
# Auto-detects the Gateway IP from the k8s cluster.
#
# Usage:
#   CF_API_TOKEN=<token> CF_ZONE_ID=<zone-id> bash cloudflare-dns.sh <team> [ip]
#
# Examples:
#   # Auto-detect Gateway IP from cluster:
#   CF_API_TOKEN=xxx CF_ZONE_ID=yyy bash cloudflare-dns.sh 00
#
#   # Override with explicit IP:
#   CF_API_TOKEN=xxx CF_ZONE_ID=yyy bash cloudflare-dns.sh 00 34.1.2.3
#
# Required env vars:
...
if [[ -z "$TEAM" ]]; then
  ...
  echo "  team : e.g. 00, 01, 02"
  echo "  ip   : optional — auto-detected from Gateway if omitted"
  exit 1
fi
