...
func callMiddleware(handler gin.HandlerFunc, token string) *httptest.ResponseRecorder {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodGet, "/api/payments/QM7X2K", nil)
	if token != "" {
...
func TestJWTAuth(t *testing.T) {
	...
	t.Run("valid RS256 token is accepted", func(t *testing.T) {
		...
		w := callMiddleware(middleware.JWTAuth(pub), token)
		assert.NotEqual(t, http.StatusUnauthorized, w.Code)
	})

	t.Run("missing Authorization header returns 401", func(t *testing.T) {
		w := callMiddleware(middleware.JWTAuth(pub), "")
		assert.Equal(t, http.StatusUnauthorized, w.Code)
	})

	t.Run("malformed token returns 401", func(t *testing.T) {
		w := callMiddleware(middleware.JWTAuth(pub), "not.a.jwt")
		assert.Equal(t, http.StatusUnauthorized, w.Code)
	})
