...
func TestMain(m *testing.M) {
	...
	schemaPath, err := filepath.Abs("../../../infra/db/qoomlee-payment/01_schema.sql")
	if err != nil {
		panic("resolve schema path: " + err.Error())
	}
	seedPath, err := filepath.Abs("../../../infra/db/qoomlee-payment/02_seed.sql")
	if err != nil {
	...
	pgc, err := postgres.Run(ctx,
		"postgres:16-alpine",
		postgres.WithDatabase("qoomlee_payment"),
