...
// seedDate returns the calendar date `days` days from now in BKK (UTC+7), for
// use with bkkDateToUTCRange. It matches the CURRENT_DATE + INTERVAL 'days'
// departure dates generated by 02_seed.sql (the postgres container's TZ is set
// to Asia/Bangkok, so CURRENT_DATE returns the BKK date).
func seedDate(days int) time.Time {
	now := time.Now().In(bkkLoc)
	return time.Date(now.Year(), now.Month(), now.Day(), 0, 0, 0, 0, bkkLoc).AddDate(0, 0, days)
}
